...
import bundledData from "../data/db.json"; // 1. Import your static data file
...
function Home() {
  ...
  useEffect(() => {
    ...
    // Local Development: Keep trying to fetch from live json-server
    const fetchProducts = async () => {
      try {
        const response = await API.get("/products");
        setFeaturedProducts(response.data.slice(0, 3));
      } catch (error) {
        console.warn("Local server down, using bundled fallback:", error);
        setFeaturedProducts(bundledData.products.slice(0, 3));
      }
    };
    fetchProducts();
