...
// 1. Use your configured API interceptor instead of raw axios
...
// 2. Establish environment switch
const getIsDev = () => {
...
function AdminDashboard() {
  ...
  const fetchProducts = async () => {
    // Production Mode (GitHub Pages): Instantly fetch from file asset and exit
    if (!isDev) {
      setProducts(database.products);
      return;
    }

    // Local Dev Mode: Keep hitting local json-server
    try {
  ...
  useEffect(() => {
    ...
    // 3. Only activate the 2-second polling interval if we are running locally!
    if (!isDev) return;
  ...
  const updatePrice = async (id, value) => {
    ...
    setProducts((prevProducts) =>
      prevProducts.map((product) =>
        product.id === id ? { ...product, price: Number(value) } : product
      )
    );

    if (!isDev) return; // Halt here in production
  ...
  const addProduct = async (e) => {
    ...
    // UI State updates instantly for responsive user feedback
    ...
    if (!isDev) {
      alert("Product Added Successfully (Production Mode)!");
      return;
    }
    ...
    try {
      ...
      console.log(error);
      alert("Error saving to local background database.");
    }
  };
    ...
    <div className="bg-gradient-to-br from-green-50 to-green-100 min-h-screen text-white">
      ...
      >
        ...
        <div className="relative z-10 text-center">
          <h1 className="text-6xl font-bold text-white">Fine Day</h1>
          <p className="text-xl mt-4 text-gray-200">Admin Dashboard</p>
        </div>
      </div>
      ...
      <div className="p-8">
        ...
        {/* ADD PRODUCT FORM */}
        {showForm && (
          <form onSubmit={addProduct} className="bg-green-700 p-6 rounded-xl shadow-lg mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {products.length > 0 ? (
            products.map((product) => (
              <div key={product.id} className="bg-green-700 p-4 rounded-xl shadow-lg">
                <img
                  src={product.image}
                  alt={product.name}
                ...
                <label className="font-semibold mt-4 block">Add Quantity</label>
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    type="button"
                    onClick={() => updateQuantity(product.id, addQuantities[product.id])}
            ...
            <h1 className="text-2xl text-gray-300">No Products Found</h1>
          )}
        </div>
      </div>
    </div>
